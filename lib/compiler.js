...
module.exports = Compiler = function(src, dst, middleware, next){
    ...
    this.compile = function() {
        logger.info('[Compiler] srcPath ' +  this.src);
        logger.info('[Compiler] dstPath ' +  this.dst);
        logger.info('[Compiler] middleware.name ' +  middleware.name);
        ...
        fs.readFile(that.src, 'utf8', function(err, str){
            ...
            try {
                ...
                logger.error("[Compiler] compile err: ");
    ...
    this.process = function() {
        ...
        fs.stat(that.dst, function(err, stat){
            if (err && 'ENOENT' != err.code){
                logger.error("[Compiler] dst stat error: code=" +  err.code);
                logger.error(err);
                that.next(err);
            } else if (err){
                // We depend on compile() to detect missing src.
                logger.info("[Compiler] dst not found");
                that.compile();
            } else {
                logger.info("[Compiler] before comp time");
