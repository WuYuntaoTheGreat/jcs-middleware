...
module.exports = function(opt){
    ...
    function calcPrefix(dst){
        return urlBase + dst.substring(opt.staticRoot.length).replace(/^\\/, '/');
    }
    ...
    if (opt.stylusSrc){
        typeMap.push({
            ...
            middleware: require('./lib/stylus-middleware')(opt)
        });
    }
    if (opt.coffeeSrc){
        typeMap.push({
            ...
            middleware: require('./lib/coffee-middleware')(opt)
        });
    }
    if (opt.jadeSrc){
        typeMap.push({
            ...
            middleware: require('./lib/jade-middleware')(opt)
        });
    }
    ...
    return function(req, res, next){
        ...
        for(var i = 0; i < typeMap.length; i++){
            if(pathname.substring(typeMap[i].prefix) === 0 && typeMap[i].suffix.text(pathname)){
                typeDef = typeMap(i);
                break;
            }
        }
